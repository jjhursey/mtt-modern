...
def check_if_phase(phase, allphases=[]):
...
def build_summary_sql(session=[], data=[]):
    fields = []
    tables = []
    tablesd = {}
    keys = {}
    ...
    if need_table("mpi_install", tablesd) == True:
        ...
        select_from_r += "\n\t\t JOIN mpi_install using (mpi_install_id)"
    if need_table("test_build", tablesd) == True:
...
def build_detail_sql(session=[], data=[]):
    fields = []
    tables = []
    tablesd = {}
    keys = {}
    ...
    if need_table("test_build", tablesd) == True and not 'test_build' in data['phases']:
        select_from_other_table += "\n\t\t JOIN test_build using (test_build_id)"
